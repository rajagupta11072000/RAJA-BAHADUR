...
const imgRaja = new Image(); imgRaja.src = 'https://i.imgur.com/8Q8zxx1.png';
...
function restartGame(){
  ...
  assets.bgMusic.play(); loop();
}

class Obstacle {
  constructor(type){
    this.type=type; this.size=60; this.x=canvas.width; this.y=canvas.height-100;
    this.speed=inBonus?12:5;
  }
  update(){ this.x-=this.speed; }
  draw(){
    const img = { soldier:imgSoldier, horse:imgHorse, elephant:imgElephant, rock:imgRock }[this.type];
...
function takeDamage(){ health--; assets.sfx.hit.play(); }
...
function update(){
  ...
  if(inBonus && Date.now()-bonusTime>20000){
    inBonus=false; assets.bonusMusic.pause(); assets.bgMusic.play();
  }
...
canvas.addEventListener('click',()=>{
  assets.bgMusic.loop=true; assets.bonusMusic.loop=true;
  assets.bgMusic.volume=0.3; assets.bonusMusic.volume=0.5;
  assets.bgMusic.play(); loop();
}, { once: true });
